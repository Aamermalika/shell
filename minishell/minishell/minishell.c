...
int built_in_check(char *line)
{
	if (!ft_strncmp("cd", line, max(ft_strlen(line), ft_strlen("cd")))\
		...
		return 12;
	else if (!ft_strncmp("export", line, max(ft_strlen(line), ft_strlen("export"))) \
		&& ft_strncmp("EXPORT", line, max(ft_strlen(line), ft_strlen("EXPORT"))))
		return 13;
		...
		return 16;
	else if (!ft_strncmp("echo", line, max(ft_strlen(line), ft_strlen("echo")))\
		 || !ft_strncmp("ECHO", line, max(ft_strlen(line), ft_strlen("ECHO"))))
		return 17;
	else
		return 0;
}
...
{
	if(!v_lines->command_table->next)
	{
		if(built_in_number == 11)
			ft_cd();
		else if(built_in_number == 13)
			ft_export(v_lines->command_table->argument);
		...
		//else if(built_in_number == 15)
			...
			ft_echo(v_lines->command_table->argument);
		//printf("execute built_in\n");
	}
}

void execute_built_in_child(int built_in_number)
{
	if(!v_lines->command_table->next)
	{
		// if(built_in_number == 11)
		// 	ft_cd();
		// if(built_in_number == 12)
		// 	ft_pwd();
		// else if(built_in_number == 14)
		// 	ft_unset();
		// else if(built_in_number == 15)
...
int process_command_table(t_line_processing_history *v_lines)
{
	...
	process_wildcards(v_lines->command_table);
	if (built_in_number)
		execute_built_in_child(built_in_number);
	return 1;
}
